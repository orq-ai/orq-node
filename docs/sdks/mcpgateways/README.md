...
async function run() {
  const result = await orq.mcpGateways.get({
    id: "<id>",
  });
...
```typescript
import { OrqCore } from "@orq-ai/node/core.js";
import { mcpGatewaysGet } from "@orq-ai/node/funcs/mcpGatewaysGet.js";
...
async function run() {
  const res = await mcpGatewaysGet(orq, {
    id: "<id>",
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("mcpGatewaysGet failed:", res.error);
  }
}
