...
<!-- UsageSnippet language="typescript" operationID="RoutingRuleList" method="get" path="/v2/routing-rules" -->
```typescript
...
import { routingRulesList } from "@orq-ai/node/funcs/routingRulesList.js";
...
async function run() {
  ...
  if (res.ok) {
    ...
    console.log("routingRulesList failed:", res.error);
  }
}
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `request`                                                                                                                                                                      | [operations.RoutingRuleListRequest](../../models/operations/routingrulelistrequest.md)                                                                                         | :heavy_check_mark:                                                                                                                                                             | The request object to use for the request.                                                                                                                                     |
...
<!-- UsageSnippet language="typescript" operationID="RoutingRuleCreate" method="post" path="/v2/routing-rules" -->
```typescript
import { Orq } from "@orq-ai/node";
...
```typescript
import { OrqCore } from "@orq-ai/node/core.js";
import { routingRulesCreate } from "@orq-ai/node/funcs/routingRulesCreate.js";
...
async function run() {
  const res = await routingRulesCreate(orq, {
    displayName: "Freeda_Beahan",
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("routingRulesCreate failed:", res.error);
  }
}
...
<!-- UsageSnippet language="typescript" operationID="RoutingRuleListUsedModels" method="get" path="/v2/routing-rules/used-models" -->
```typescript
import { Orq } from "@orq-ai/node";
...
```typescript
import { OrqCore } from "@orq-ai/node/core.js";
import { routingRulesListUsedModels } from "@orq-ai/node/funcs/routingRulesListUsedModels.js";
...
async function run() {
  const res = await routingRulesListUsedModels(orq);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("routingRulesListUsedModels failed:", res.error);
  }
}
...
<!-- UsageSnippet language="typescript" operationID="RoutingRuleDelete" method="delete" path="/v2/routing-rules/{routing_rule_id}" -->
```typescript
import { Orq } from "@orq-ai/node";
...
```typescript
import { OrqCore } from "@orq-ai/node/core.js";
import { routingRulesDelete } from "@orq-ai/node/funcs/routingRulesDelete.js";
...
async function run() {
  const res = await routingRulesDelete(orq, {
    routingRuleId: "<id>",
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("routingRulesDelete failed:", res.error);
  }
}
...
| Parameter                                                                                                                                                                      | Type                                                                                                                                                                           | Required                                                                                                                                                                       | Description                                                                                                                                                                    |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `request`                                                                                                                                                                      | [operations.RoutingRuleDeleteRequest](../../models/operations/routingruledeleterequest.md)                                                                                     | :heavy_check_mark:                                                                                                                                                             | The request object to use for the request.                                                                                                                                     |
| `options`                                                                                                                                                                      | RequestOptions                                                                                                                                                                 | :heavy_minus_sign:                                                                                                                                                             | Used to set various options for making HTTP requests.                                                                                                                          |
| `options.fetchOptions`                                                                                                                                                         | [RequestInit](https://developer.mozilla.org/en-US/docs/Web/API/Request/Request#options)                                                                                        | :heavy_minus_sign:                                                                                                                                                             | Options that are passed to the underlying HTTP request. This can be used to inject extra headers for examples. All `Request` options, except `method` and `body`, are allowed. |
| `options.retries`                                                                                                                                                              | [RetryConfig](../../lib/utils/retryconfig.md)                                                                                                                                  | :heavy_minus_sign:                                                                                                                                                             | Enables retrying HTTP requests under certain failure conditions.                                                                                                               |
...
<!-- UsageSnippet language="typescript" operationID="RoutingRuleUpdate" method="patch" path="/v2/routing-rules/{routing_rule_id}" -->
```typescript
import { Orq } from "@orq-ai/node";
...
```typescript
import { OrqCore } from "@orq-ai/node/core.js";
import { routingRulesUpdate } from "@orq-ai/node/funcs/routingRulesUpdate.js";
...
async function run() {
  ...
  if (res.ok) {
    ...
    console.log("routingRulesUpdate failed:", res.error);
  }
}
...
| Parameter                                                                                                                                                                      | Type                                                                                                                                                                           | Required                                                                                                                                                                       | Description                                                                                                                                                                    |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `request`                                                                                                                                                                      | [operations.RoutingRuleUpdateRequest](../../models/operations/routingruleupdaterequest.md)                                                                                     | :heavy_check_mark:                                                                                                                                                             | The request object to use for the request.                                                                                                                                     |
| `options`                                                                                                                                                                      | RequestOptions                                                                                                                                                                 | :heavy_minus_sign:                                                                                                                                                             | Used to set various options for making HTTP requests.                                                                                                                          |
| `options.fetchOptions`                                                                                                                                                         | [RequestInit](https://developer.mozilla.org/en-US/docs/Web/API/Request/Request#options)                                                                                        | :heavy_minus_sign:                                                                                                                                                             | Options that are passed to the underlying HTTP request. This can be used to inject extra headers for examples. All `Request` options, except `method` and `body`, are allowed. |
| `options.retries`                                                                                                                                                              | [RetryConfig](../../lib/utils/retryconfig.md)                                                                                                                                  | :heavy_minus_sign:                                                                                                                                                             | Enables retrying HTTP requests under certain failure conditions.                                                                                                               |
