/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod/v3";
import { remap as remap$ } from "../../lib/primitives.js";
import {
  collectExtraKeys as collectExtraKeys$,
  safeParse,
} from "../../lib/schemas.js";
import { Result as SafeParseResult } from "../../types/fp.js";
import { SDKValidationError } from "../errors/sdkvalidationerror.js";

/**
 * A `response.code_interpreter_call_code.done` server-sent event.
 */
export type ResponseCodeInterpreterCallCodeDoneStreamEvent = {
  /**
   * The completed code interpreter code.
   */
  code: string;
  /**
   * ID of the output item this event refers to.
   */
  itemId: string;
  /**
   * Index of the output item in the response output array.
   */
  outputIndex: number;
  /**
   * Monotonically increasing sequence number for ordering events.
   */
  sequenceNumber: number;
  /**
   * The event type. Discriminates the payload.
   */
  type: "response.code_interpreter_call_code.done";
  additionalProperties?: { [k: string]: any } | undefined;
};

/** @internal */
export const ResponseCodeInterpreterCallCodeDoneStreamEvent$inboundSchema:
  z.ZodType<
    ResponseCodeInterpreterCallCodeDoneStreamEvent,
    z.ZodTypeDef,
    unknown
  > = collectExtraKeys$(
    z.object({
      code: z.string(),
      item_id: z.string(),
      output_index: z.number().int(),
      sequence_number: z.number().int(),
      type: z.literal("response.code_interpreter_call_code.done"),
    }).catchall(z.any()),
    "additionalProperties",
    true,
  ).transform((v) => {
    return remap$(v, {
      "item_id": "itemId",
      "output_index": "outputIndex",
      "sequence_number": "sequenceNumber",
    });
  });

export function responseCodeInterpreterCallCodeDoneStreamEventFromJSON(
  jsonString: string,
): SafeParseResult<
  ResponseCodeInterpreterCallCodeDoneStreamEvent,
  SDKValidationError
> {
  return safeParse(
    jsonString,
    (x) =>
      ResponseCodeInterpreterCallCodeDoneStreamEvent$inboundSchema.parse(
        JSON.parse(x),
      ),
    `Failed to parse 'ResponseCodeInterpreterCallCodeDoneStreamEvent' from JSON`,
  );
}
