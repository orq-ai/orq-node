/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod/v3";
import { remap as remap$ } from "../../lib/primitives.js";
import {
  collectExtraKeys as collectExtraKeys$,
  safeParse,
} from "../../lib/schemas.js";
import { Result as SafeParseResult } from "../../types/fp.js";
import { SDKValidationError } from "../errors/sdkvalidationerror.js";

/**
 * A `response.image_generation_call.generating` server-sent event.
 */
export type ResponseImageGenerationCallGeneratingStreamEvent = {
  /**
   * ID of the output item this event refers to.
   */
  itemId: string;
  /**
   * Index of the output item in the response output array.
   */
  outputIndex: number;
  /**
   * Monotonically increasing sequence number for ordering events.
   */
  sequenceNumber: number;
  /**
   * The event type. Discriminates the payload.
   */
  type: "response.image_generation_call.generating";
  additionalProperties?: { [k: string]: any } | undefined;
};

/** @internal */
export const ResponseImageGenerationCallGeneratingStreamEvent$inboundSchema:
  z.ZodType<
    ResponseImageGenerationCallGeneratingStreamEvent,
    z.ZodTypeDef,
    unknown
  > = collectExtraKeys$(
    z.object({
      item_id: z.string(),
      output_index: z.number().int(),
      sequence_number: z.number().int(),
      type: z.literal("response.image_generation_call.generating"),
    }).catchall(z.any()),
    "additionalProperties",
    true,
  ).transform((v) => {
    return remap$(v, {
      "item_id": "itemId",
      "output_index": "outputIndex",
      "sequence_number": "sequenceNumber",
    });
  });

export function responseImageGenerationCallGeneratingStreamEventFromJSON(
  jsonString: string,
): SafeParseResult<
  ResponseImageGenerationCallGeneratingStreamEvent,
  SDKValidationError
> {
  return safeParse(
    jsonString,
    (x) =>
      ResponseImageGenerationCallGeneratingStreamEvent$inboundSchema.parse(
        JSON.parse(x),
      ),
    `Failed to parse 'ResponseImageGenerationCallGeneratingStreamEvent' from JSON`,
  );
}
