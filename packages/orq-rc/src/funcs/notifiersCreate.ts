 ...
 */
export function notifiersCreate(
  client: OrqCore,
  request: components.CreateNotifierRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.CreateNotifierResponse,
    | OrqError
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CreateNotifierRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.CreateNotifierResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
