...
export function monitorsCreate(
  ...
  Result<
    components.CreateMonitorResponse,
    | OrqError
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CreateMonitorRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v2/monitors")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.CreateMonitorResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
