...
> {
  const parsed = safeParse(
    request,
    (value) => components.SearchLogsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v3/logs/search")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SearchLogsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
