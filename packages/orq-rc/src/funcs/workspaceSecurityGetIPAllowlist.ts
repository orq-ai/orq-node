 ...
 */
export function workspaceSecurityGetIPAllowlist(
  client: OrqCore,
  request: operations.WorkspaceSecurityGetIPAllowlistRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.GetIPAllowlistResponse,
    | OrqError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.WorkspaceSecurityGetIPAllowlistRequest$outboundSchema.parse(
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.GetIPAllowlistResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
