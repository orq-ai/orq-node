 ...
 * Update a dataset
 */
export function datasetsUpdate(
  client: OrqCore,
  request: operations.UpdateDatasetRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.UpdateDatasetResponseBody,
    | errors.HonoApiError
    | APIError
    | SDKValidationError
    | UnexpectedClientError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateDatasetRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.UpdateDatasetResponseBody$inboundSchema),
