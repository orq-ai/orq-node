...
export function smartRoutersList(
  ...
  Result<
    components.ListSmartRoutersResponse,
    | OrqError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.SmartRouterListRequest$outboundSchema.optional().parse(value),
  ...
  const path = pathToFunc("/v2/smart-routers")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ListSmartRoutersResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
