...
export function workspaceSettingsUpdate(
  ...
  Result<
    components.UpdateWorkspaceSettingsResponse,
    | OrqError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      components.UpdateWorkspaceSettingsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v2/workspace-settings")();
  ...
  const context = {
    ...
    operationID: "WorkspaceSettingsUpdate",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.UpdateWorkspaceSettingsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
