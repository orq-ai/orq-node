 ...
 */
export function logsListTraceLogs(
  client: OrqCore,
  request: operations.ListTraceLogsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ListTraceLogsResponse,
    | OrqError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListTraceLogsRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ListTraceLogsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
