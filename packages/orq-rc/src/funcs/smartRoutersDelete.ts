...
export function smartRoutersDelete(
  ...
  Result<
    components.DeleteSmartRouterResponse,
    | OrqError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.SmartRouterDeleteRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.DeleteSmartRouterResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
