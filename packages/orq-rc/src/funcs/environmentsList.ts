...
export function environmentsList(
  ...
  Result<
    components.ListEnvironmentsResponse,
    | OrqError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.EnvironmentListRequest$outboundSchema.optional().parse(value),
  ...
  const path = pathToFunc("/v3/environments")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ListEnvironmentsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
