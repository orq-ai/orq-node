...
export function annotationQueuesAddItems(
  ...
  Result<
    components.AddAnnotationQueueItemsResponse,
    | OrqError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AddAnnotationQueueItemsRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AddAnnotationQueueItemsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
