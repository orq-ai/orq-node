...
export function alertsCreate(
  ...
  Result<
    components.CreateAlertResponse,
    | OrqError
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CreateAlertRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v2/alerts")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.CreateAlertResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
