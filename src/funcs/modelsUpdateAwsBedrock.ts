 ...
 */
export function modelsUpdateAwsBedrock(
  client: OrqCore,
  request: operations.ModelUpdateAwsBedrockRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.ModelUpdateAwsBedrockResponseBody,
    | OrqError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ModelUpdateAwsBedrockRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v2/models/aws-bedrock/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ModelUpdateAwsBedrockResponseBody$inboundSchema),
    M.fail([400, 404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
