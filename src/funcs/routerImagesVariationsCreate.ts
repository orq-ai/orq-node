 ...
 */
export function routerImagesVariationsCreate(
  client: OrqCore,
  request: operations.CreateImageVariationRequestBody,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.CreateImageVariationResponseBody,
    | OrqError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateImageVariationRequestBody$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v2/router/images/variations")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.CreateImageVariationResponseBody$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
