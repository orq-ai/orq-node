...
> {
  const parsed = safeParse(
    request,
    (value) => components.QueryLogsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v3/logs/query")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.QueryLogsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
