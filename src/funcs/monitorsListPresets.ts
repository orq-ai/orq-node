 ...
 */
export function monitorsListPresets(
  client: OrqCore,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ListMonitorPresetsResponse,
    | OrqError
...
> {
  const path = pathToFunc("/v2/monitors/presets")();
  ...
  const context = {
    ...
    operationID: "MonitorListPresets",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ListMonitorPresetsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
