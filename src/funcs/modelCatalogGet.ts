...
export function modelCatalogGet(
  ...
  Result<
    components.GetModelCatalogModelResponse,
    | OrqError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ModelCatalogGetRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v2/model-catalog/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.GetModelCatalogModelResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
