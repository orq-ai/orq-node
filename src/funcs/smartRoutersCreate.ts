...
export function smartRoutersCreate(
  ...
  Result<
    components.CreateSmartRouterResponse,
    | OrqError
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CreateSmartRouterRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.CreateSmartRouterResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
