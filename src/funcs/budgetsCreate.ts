...
export function budgetsCreate(
  ...
  Result<
    components.CreateBudgetResponse,
    | OrqError
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CreateBudgetRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.CreateBudgetResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
